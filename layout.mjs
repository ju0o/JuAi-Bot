...
**어디서나**
• \`리뷰해줘 https://github.com/아이디/저장소\` → 🔵 OpenCode가 저장소를 보고 코드 리뷰 (1회). 프로필에 GitHub를 적어두면 \`리뷰해줘\`만 써도 돼요
...
export const STAFF_GUIDE = (c) => `## 🛠️ 운영진 안내 (Founder 전용)
Claude Code를 켜지 않아도 **이 채널만으로 운영**돼요. 봇은 ASUS가 켜져 있으면 알아서 돌아가요.

**📱 하루 루틴 (5분)**
1. 여기 올라온 카드에 **승인 · 보류 · 더 생각** 누르기
2. ${c("summary")}에서 어제 대화 3줄 요약 보기
3. 시간 되면 ${c("playground")} 봇 수다 구경

**🗣️ 여기에 말로 시키기** (다른 채널에선 @Claude를 붙여서)
• "봇-놀이터 채널 설명 바꿔줘" / "요즘 무슨 얘기 많아?" → 바로 처리
• "이번 주 공지 써줘" / "규칙에 ○○ 추가해줘" → 초안 카드
• "자유대화 광고 글 지워줘" / "OO 1시간 타임아웃" / "채널 만들어줘·삭제해줘" → 확인 카드
• "하루 한도 7회로 올려줘" / "○○ 기능 넣어줘" → 봇이 직접 코드 수정 → 적용 카드
• "GitHub 릴리즈 올려줘" → 새 버전 공개 + ${c("source")}에 릴리즈 노트

**🔘 버튼 뜻**
승인 = 바로 실행 · 보류 = 일주일 뒤 다시 물어봄 · 더 생각 = 의견 한 줄 쓰면 고친 안으로 다시

**🔔 자동으로 올라오는 카드**
월 10시 운영 리포트·건의 TOP 5 · 금 18시 하이라이트 · 매달 1일 피드백 장인 · 변경이 쌓이면 "지금 올리면 될 거 같아요" 릴리즈 · 스팸 조치

**🙋 Claude Code를 켜야 할 때**
봇이 대답을 안 할 때(먼저 ${c("botlog")} 확인) · 디스코드 밖의 일(영상, SNS 게시) · "기능 넣어줘"로 안 되는 큰 변경
-# 새 기능이 켜지지 않으면 이전 버전으로 자동 복구돼요.`;
